...
export default function HeroSection({ heading, subheading, showLogo = false, fitParentHeight = false }: HeroSectionProps) {
  return (
    <div className={`relative w-full bg-[#003C54] flex flex-col pb-8 md:pb-0 md:justify-end ${fitParentHeight ? "h-full min-h-0" : "min-h-screen"}`}>
      <div className={`flex-1 flex flex-col items-start justify-end px-8 md:px-16 lg:px-32 xl:px-58 ${fitParentHeight ? "pb-4 md:pb-6 lg:pb-8" : "pb-8 md:pb-12 lg:pb-16"}`}>
        <motion.div
        ...
        >
          ...
          {showLogo && (
            <motion.div
              ...
              className={`${fitParentHeight ? "mb-3 md:mb-4 lg:mb-5" : "mb-5 md:mb-7 lg:mb-8"}`}
            >
              <Image
                ...
                width={150}
                height={150}
                className={`${fitParentHeight ? "w-16 md:w-20 lg:w-24 xl:w-28" : "w-20 md:w-28 lg:w-32 xl:w-36"} h-auto`}
              />
            </motion.div>
          )}
          <h1
